...
// Bear sprite (side view)
static vector<string> BEAR = {
    "   (BEAR)   ",
    "   /^--^\\   ",
    "  ( o  o )  ",
    "   > == <   ",
...
static vector<string> BEAR_HURT = {
    "   (BEAR)   ",
    "   /^--^\\   ",
    "  ( x  x )  ", // eyes hurt
    "   > == <   ",
...
static vector<string> BEAR_LUNGE = {
    "   (BEAR)   ",
    "   /^--^\\   ",
    "  ( o  o )  ",
    "   > == <   ", // maybe tilt head forward slightly if you like
    "   (_____)  "
};
...
void battleArenaBearForest() {
    ...
    auto lastBearMove = lastFrame;
    auto lastBearLunge = lastFrame;
    auto lastBearAttack = lastFrame;
        ...
        if (key != -1) {
            if (key >= 1000) {
                ...
                // H = Slash (3-tile wide in facing direction)
                    ...
                    stamp(base, BEAR, bearX, bearY);
                    stamp(base, BEAR, bearX, bearY); // draw enemy so effect is visible beneath
                ...
                //Sword Hit
                    ...
                    for (auto& f : frames) {
                        ...
                        stamp(buf, BEAR, bearX, bearY);      // draw villain first
...
// enemy AI: follow in 4 directions (simple)
        if (chrono::duration_cast<chrono::milliseconds>(now - lastBearMove).count() >= 160) {
            if (bearX < playerX) bearX++;
            else if (bearX > playerX) bearX--;
            if (bearY < playerY) bearY++;
            else if (bearY > playerY) bearY--;
            lastBearMove = now;
        }

        // enemy lunge occasionally if near but not too near
        if (chrono::duration_cast<chrono::milliseconds>(now - lastBearLunge).count() >= 1800) {
            int dist = abs(bearX - playerX) + abs(bearY - playerY);
            if (dist <= 8 && dist >= 3) {
                ...
                for (int s = 0; s < 3; s++) {
                    ...
                    stamp(f, BEAR_LUNGE, bearX, bearY);
        ...
        for (auto& p : projectiles) {
            ...
            // collision
            if (abs(p.x - bearX) <= 1 && abs(p.y - bearY) <= 1 && bearHP > 0) {
                bearHP = max(0, bearHP - p.dmg);
                p.active = false;
                Beep(800, 100);
            }
        ...
        // draw villain first
        if (bearHP > 0) stamp(frame, BEAR, bearX, bearY);
        else stamp(frame, BEAR_HURT, bearX, bearY);
